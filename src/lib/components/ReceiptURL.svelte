<script lang="ts">
    ...
    const startSplit = async () => {
        if (!urlValid) {
            error = 'Please enter a valid receipt URL.';
            return;
        }
        error = null;
        try {
...
<div class="mx-auto mt-8 max-w-xl p-4">
    ...
    <p class="mb-4 text-gray-600">
        Enter the URL of your receipt to split it among your friends. Make sure the URL is
        correct and points to a valid receipt.
    </p>

    <label class="mb-2 block font-medium text-gray-700" for="receipt-url"> Receipt URL </label>
    <input
        id="receipt-url"
        type="text"
        ...
        bind:value={url}
        placeholder="https://www.receipturl.com/receipts/..."
    ...
    <p class="mt-1 text-sm {!urlValid && url.length > 0 ? 'text-red-600' : 'text-gray-500'}">
        {#if !urlValid && url.length > 0}
            Please enter a valid, supported receipt URL.
        {:else}
            Enter the URL of your receipt to parse it.
        {/if}
    </p>
