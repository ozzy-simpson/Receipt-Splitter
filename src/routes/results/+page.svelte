	...
	<div class="mx-6 my-10 max-w-xl rounded-xl border border-gray-300 bg-gray-100 p-6 shadow-lg">
		...
		<div class="mb-6 flex flex-wrap justify-center gap-4">
			{#each Array.from(peopleSet) as person, idx}
				<div class="flex w-full items-center gap-3 rounded-lg bg-white px-4 py-2 shadow sm:w-auto">
					<span
			...
			{#if receipt.tax || receipt.tip || receipt.fees}
				<span class="text-sm text-gray-500">
					(Tax: {new Intl.NumberFormat('en-US', { style: 'currency', currency: 'USD' }).format(
						receipt.tax || 0
					)}, Tip: {new Intl.NumberFormat('en-US', { style: 'currency', currency: 'USD' }).format(
						receipt.tip || 0
					)}, Fees: {new Intl.NumberFormat('en-US', { style: 'currency', currency: 'USD' }).format(
						receipt.fees || 0
					)})
				</span>
			{/if}
